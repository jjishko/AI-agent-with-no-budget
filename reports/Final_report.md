...
* action, symptom и time_range: возможные слова и конструкции, отражающие данную сущность12
...
Такой подход оказался хорошим для некоторых сущностей, однако отмечено много неоднозначностей и ошибок. Подробнее в отчете error_analysis.md.
## Задача классификации
# ML/LLM-подходы
## Задача NER

Для более сложной модели выбран ruBERT, так как он способен обрабатывать русский язык и сложные конструкции, одновременно являясь простым и классическим вариантом распознавания сущностей. Параметры подобраны с помощью Optuna через кросс-валидацию - 5 проходов и 5 фолдов для начала. Настраиваемые гиперпараметры (оптимальные параметры будут добавлены позже):
...
* Клиппинг градиентов
 ...
 Такой подход уменьшил количество ошибок, связанных с разнообразием языка. Подробнее в отчете error_analysis.md.
## Задача классификации
# Сравнение подходов
## Задача NER

По результатам сравнения метрик и ошибок, RuBERT справляется с распознаванием сущностей однозначно лучше классического rule-based, что обуславливает его использование. Но стоит отметить, что правила тоже имеют преимущества.

**Rule-based**: плохо справляется с контекстными сущностями action, symptom и прочими сложными случаями, однако обрабатываются теги, оборудование и другие похожие сущности с помощью регулярок.

**ruBERT**: хуже находит сущности, представляющиеся списком конечных элементов, но умеет читать контекст, выявляя такие сущности, как action.

Применим вариант объединения двух моделей для лучшего качества.
## Задача классификации
# Выводы

Были разработаны модели для распознавания сущностей и пользовательских намерений. В результате получен семантический роутер, анализирующий запрос и направляющий соответствующему агенту. Это позволяет ускорить генерацию ответа на вопросы пользователей и предоставить текст в структурированной форме.

Ошибки и неточности в моделях на допустимом уровне. Для улучшения качества следует добавить больше примеров для обучения и исправить разметки. Также стоит обратить внимание на похожие запросы, которые могли завысить результате при тесте.
# Рекомендации по внедрению

Прототип можно использовать для классических вопросов по оборудованию и документации (RAG), а также для анализа временных рядов по тегам. Необходима дальнейшая работа для полного внедрения в промышленные предприятия. 

Рекомендуется регулярно обновлять датасет запросов и обучать модели для достижения актуальных и качественных результатов.
# Ограничения

* Избегать опечаток
* Строить точные и стандартные запросы
* Избегать двусмысленности
# Дальнейшие действия

* Дополнить данные для снижения дисбаланса, охватить больше различных примеров интентов и сущностей
* Исключить похожие запросы, предотвратив вероятную утечку 
* Добавить дополнительные сущности: раздел документа, метод для вычислений, вид представляемой информации (графики, отчеты), версия (документа, модели), местоположение (по карте объекта)
* Проверить разметку 
* Рассмотреть другие модели
* Расширить калибровку параметров для достижения оптимальных настроек
* Улучшать нормализацию по мере возникновения неточностей
* Добавить справочник сущностей и линкинг
* Рассмотреть дополнительные метрики оценки качества
